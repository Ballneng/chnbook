...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    $sql = "SELECT COUNT(*) FROM ".$ecs->table('feedback')." WHERE parent_id=0";
    $all_question_num = $db->GetOne($sql);
    
   	$sql = "SELECT COUNT(*) FROM (".
   	"SELECT DISTINCT parent_id FROM ".$ecs->table('feedback')." f WHERE parent_id<>0".
   	") f , ".$ecs->table('feedback')." r WHERE f.parent_id=r.msg_id AND r.parent_id=0";
   	$answer_num = $db->GetOne($sql);
   	$smarty->assign('feedback_number',$all_question_num-$answer_num);
...
}elseif($_REQUEST['act'] == 'seotongji'){
?>
	<html>
	<body>

	<form action="index.php?act=seotongjidown" method="post"
...
<?php
}elseif ($_REQUEST['act'] == 'seotongjidown'){
if($_FILES["file"]["type"] == 'text/plain'){
      ...
      {
		chmod("./tmp/" . $_FILES["file"]["name"],0777); 
      }
	...
	foreach($res as $key=>$val){
		$sql = "SELECT pay_time,add_time FROM ecs_order_info WHERE order_sn = '{$val}'";
		if($time = $db->getAll($sql)){
			$arr[$n]["pay_time"] = $time[0]["pay_time"];
			$arr[$n]["add_time"] = $time[0]["add_time"];
			$arr[$n]["order_sn"] = $val;
		}else{
			$arr[$n]["pay_time"] = $time[0]["pay_time"];
			$arr[$n]["add_time"] = $time[0]["add_time"];
