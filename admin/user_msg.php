...
function msg_list()
{
    ...
    $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'msg_id' : trim($_REQUEST['sort_by']);
    $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

    $where = ' AND user_id!=9999999 ';
    if ($filter['keywords'])
    {
...
/*    $sql = "SELECT f.msg_id, f.user_name, f.msg_title, f.msg_type, f.order_id, f.msg_status, f.msg_time, f.msg_area, COUNT(r.msg_id) AS reply " .
    ...
    //优化后的sql
    $sql = "SELECT f.msg_id, f.user_name, f.msg_title, f.msg_type, f.order_id, f.msg_status, f.msg_time, f.msg_area ,COUNT(r.msg_id) AS reply ".
    		"FROM " . $GLOBALS['ecs']->table('feedback') . " AS r ".
    		"RIGHT JOIN ".
    		"(".
    		"SELECT * FROM " . $GLOBALS['ecs']->table('feedback') .
            "WHERE parent_id = 0 $where " .
            "ORDER by $filter[sort_by] $filter[sort_order] ".
            "LIMIT " . $filter['start'] . ', ' . $filter['page_size'].
            ") f ".
