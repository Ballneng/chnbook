...
//-- 订单查询
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'njphp_quick_ship'){

	include_once('njphp_quick_ship.inc.php');

	$smarty->assign('ur_here', '一键确认并发货');
	$smarty->display('njphp_quick_ship.htm');
...
{
	if($_REQUEST['act_type'] == 'exprot')
	{
		$export_list = order_export_list($_LANG);
		$kn = array(
			'goods_name' => '商品名称',
			'order_sn' => '订单号',
			'buyer' => '用户',
			'short_order_time' => '下单时间',
			'consignee' => '收货人',
			'tel' => '电话',
			'mobile' => '手机',
			'address' => '地址',
			'shipping_name' => '配送方式',
			'invoice_no' => '发货单号',
			'formated_total_fee' => '总金额',
			'all_order_status' => '订单状态',
		);
		foreach($kn as $key => $val)
		{
			$kn[$key] = iconv('GB2312', 'UTF-8', $val);
		}
		$name = "order_".date('YmdHi');
		down_xls($export_list,$kn,$name);
	}
    /* 检查权限 */
    admin_priv('order_view');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('shipping' == $step)
    {
        ...
        $payment_support_cod = $payment_info['is_cod'] ? true : false;

        /* 取得配送费用 */
        $total = order_weight_price($order_id);
        foreach ($shipping_list AS $key => $shipping)
        {
            /*if ($payment_support_cod != (bool)$shipping['support_cod'])
            {
    ...
    elseif ('payment' == $step)
    {
        ...
        if (exist_real_goods($order_id))
        {
            ...
            {
                if ($shipping_area['support_cod'] != (bool)$payment['is_cod']) {
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        //NJPHP
        $filter['shipping_name'] = empty($_REQUEST['shipping_name']) ? '' : trim($_REQUEST['shipping_name']);
        ...
        //NJPHP
        if ($filter['shipping_name'] && $filter['shipping_name'] != -1)
        {
            $where .= " AND o.shipping_name LIKE '%" . mysql_like_quote($filter['shipping_name']) . "%'";
        }
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, o.shipping_name, o.invoice_no, " .
...
function order_export_list($_LANG)
{
    $result = get_filter();
    if ($result === false)
    {
        ...
        $sql = "SELECT g.goods_name,o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel,o.mobile, o.extension_code, o.extension_id, o.shipping_name, o.invoice_no, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
                " FROM " . $GLOBALS['ecs']->table('order_goods') . " AS g " . 
        		" LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . " AS o ON g.order_id=o.order_id " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ". $where .
                " ORDER BY $filter[sort_by] $filter[sort_order] ";
    ...
    foreach ($row AS $key => $value)
    {
    	$row[$key]['goods_name'] = iconv('GB2312', 'UTF-8', $row[$key]['goods_name']);
    	$row[$key]['buyer'] = iconv('GB2312', 'UTF-8', $row[$key]['buyer']);
    	$row[$key]['consignee'] = iconv('GB2312', 'UTF-8', $row[$key]['consignee']);
    	$row[$key]['address'] = iconv('GB2312', 'UTF-8', $row[$key]['address']);
    	$row[$key]['shipping_name'] = iconv('GB2312', 'UTF-8', $row[$key]['shipping_name']);
    	$row[$key]['all_order_status'] = iconv('GB2312', 'UTF-8', $row[$key]['all_order_status']);
    	
        $row[$key]['formated_order_amount'] = price_format($value['order_amount']);
        $row[$key]['formated_money_paid'] = price_format($value['money_paid']);
        $row[$key]['formated_total_fee'] = iconv('GB2312', 'UTF-8', price_format($value['total_fee']));
        $row[$key]['short_order_time'] = local_date('m-d H:i', $value['add_time']);
        if ($value['order_status'] == OS_INVALID || $value['order_status'] == OS_CANCELED)
        {
            /* 如果该订单为无效或取消则显示删除链接 */
        ...
        $row[$key]['all_order_status'] = iconv('GB2312', 'UTF-8', $_LANG['os'][$row[$key]['order_status']] . $_LANG['ps'][$row[$key]['pay_status']] . $_LANG['ss'][$row[$key]['shipping_status']]);
    }
