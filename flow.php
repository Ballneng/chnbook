...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    //njphp fix for user address 2011 09 09
	$consignee['njphp_provice']  = $GLOBALS['db']->GetOne("SELECT region_name FROM ".$GLOBALS['ecs']->table('region') ." WHERE region_id = '$consignee[province]'");
	$consignee['njphp_city']     = $GLOBALS['db']->GetOne("SELECT region_name FROM ".$GLOBALS['ecs']->table('region') ." WHERE region_id = '$consignee[city]'");
	$consignee['njphp_district'] = $GLOBALS['db']->GetOne("SELECT region_name FROM ".$GLOBALS['ecs']->table('region') ." WHERE region_id = '$consignee[district]'");
    ...
    //2012-06-30    qianlei   修改货运方式
    $baoyou = false;
    if (is_goods_baoyou($cart_goods))
    {
        $shipping_list =  choose_shipping_by_baoyou($shipping_list);
        $baoyou = true;
    }
	else
	{
		//if($_SESSION['user_id']==18184){print_r($shipping_list);}
        //韵达，EMS快递，宅急送，顺丰
        /*$tmp_shipping = array();
        $shipping_order = array('yto','ems','zto','sto_express','sf_express');
        foreach ($shipping_order as $k)
        {
            foreach ($shipping_list as $key => $_shipping)
            {
                if ($_shipping['shipping_code']==$k){
                    $tmp_shipping[] = $_shipping;
                    break;
                }
            }
        }
        $shipping_list = $tmp_shipping;
        unset($tmp_shipping,$shipping_order);*/

		//不是包邮教材去掉包邮快递
		foreach ($shipping_list as $key=>$row)
		{
			if ($row['shipping_id']==15)
			{
    ...
    /*2012-08-19 添加服务范围*/
    //$shipping_list = shipping_add_service_range($shipping_list);

    //2012-09-05   qianlei  福建龙岩 特殊处理,因为
    //包邮教材内出现福建龙岩的话，直接把包邮快递换为EMS快递，这个改一个名字就可以了，因为这个EMS快递是包邮的，
    //非包邮教材的内福建龙岩的话就直接出现EMS快递
    $region_in_longyan = $consignee['province']==4 && $consignee['city']==54 ? true : false;
    if ($region_in_longyan)
    {
        if ($baoyou)
        {
            foreach ($shipping_list as $key => $_shipping)
            {
                if ($_shipping['shipping_code'] == 'post_express')
                {
                    $shipping_list[$key]['shipping_name'] = 'EMS 国内邮政特快专递（<font color="red">包邮</font>）';
                    $shipping_list[$key]['shipping_desc'] = '可登陆EMS 国内邮政特快专递网站http://www.ems.com.cn/，查询跟踪快件运送情况';
                }
            }
        }
        else
        {
            foreach ($shipping_list as $key => $_shipping)
            {
                if ($_shipping['shipping_code'] == 'yto')
                    unset($shipping_list[$key]);
            }
    ...
    /*2012-08-19 qianlei 添加支付特别提示*/
    $payment_list = payment_add_special_tips($payment_list);
    ...
    //qianlei
    $smarty->assign('alipaybank_list',get_alipaybank_list());
    if ($order['pay_id'])
    {
        $sql = "SELECT pay_code FROM " . $ecs->table('payment') ." WHERE pay_id = '" . $order['pay_id'] . "' AND enabled=1" ;
        if ($db->getOne($sql) == 'alipaybank')
        {
            $smarty->assign('show_alipaybank_list',true);
            $smarty->assign('selected_alipaybank',isset($_SESSION['alipaybank_code']) ? $_SESSION['alipaybank_code'] : '');
        }
    }
...
elseif ($_REQUEST['step'] == 'select_alipaybank') //支付宝网银支付选择银行
{
    $f = 'err';
    $bankcode = isset($_POST['code']) ? strtoupper(trim($_POST['code'])) : '';
    $banklist = get_alipaybank_list();
    if ($bankcode && isset($banklist[$bankcode]))
    {
        $_SESSION['alipaybank_code'] = $bankcode;
        $f = 'succ';
    }
    echo $f;
    exit;
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	$have_jiangsu = false;
	/*$ishave = false;
	$baoyou_arr = array(45,40,10,4,5,6,9,30,15,16,31,35);
	
	foreach ($cart_goods_stock['goods_list'] as $value){
		if($value["goods_id"] == 42){
			$have_jiansu_str = "<font style='color:red;'><p>本次包邮活动仅限于非货到付款方式订购2012年江苏公务员考试通用教材的考生。<br>考生下单后请及时联系客服热线400-0677-898修改订单金额，付款成功后会在第一时间安排发货。</p>
<p>备注：包邮活动的快递公司不以考生下单选择为准，由教材中心统一安排。</p></font>";
			$have_jiangsu = true;
			$ishave = true;
		}
		
		if(in_array($value["goods_id"],$baoyou_arr) && !$ishave){
			$have_jiansu_str = "<font style='color:red;'><p>本次包邮活动仅限于非货到付款方式订购事业单位考试复习教材、河南、广东、江西、湖北、福建、浙江、四川、内蒙古、重庆、吉林、西藏等11省公务员考试教材的考生，</br>考生下单后请及时联系客服热线400-0677-898修改订单运费金额，付款成功后会在第一时间安排发货。</p>
<p>备注：包邮活动的快递公司不以考生下单选择为准，由教材中心统一安排。</br>偏远地区只能通过EMS快递邮寄，包邮金额限制为20元内，超出部分需要您支付运费差额。</p></font>";
			$have_jiangsu = true;
		}elseif(in_array($value["goods_id"],$baoyou_arr) && $ishave){
			$have_jiansu_str = "<font style='color:red;'><p>考生下单后请及时联系客服热线400-0677-898修改订单运费金额，付款成功后会在第一时间安排发货。</p>
<p>备注：包邮活动的快递公司不以考生下单选择为准，由教材中心统一安排。</p></font>";
			$have_jiangsu = true;
		}
	}*/
        
	//2012-06-30    qianlei    只能买一件商品,且该商品属于包邮产品（已经和客服商量）
	if (is_goods_baoyou($cart_goods_stock['goods_list']))
	{
		$have_jiangsu = true;
		$have_jiansu_str = '<font color="red">本次包邮活动仅限于非货到付款方式订购事业单位考试复习教材、江苏、河南、广东、江西、湖北、福建、浙江、四川、内蒙古、重庆、吉林、西藏等12省公务员考试教材的考生，付款成功后会在第一时间安排发货。
   备注：包邮活动的快递公司由教材中心统一安排。偏远地区只能通过EMS快递邮寄，包邮金额限制为20元内，超出部分需要您支付运费差额。</font>';
    }
	/*********************江苏图书包邮活动********************/
   ...
   /* 判断是不是实体商品 */
    ...
    {
        show_message($_LANG['flow_no_payment']);
    }
    $order['pay_fee'] = $total['pay_fee'];
    $order['cod_fee'] = $total['cod_fee'];

	//判断配送方式和支付放松是否匹配 qianlei 2013-02-26
    if ($shipping['support_cod']!=$payment['is_cod'])
    {
        if ($shipping['support_cod']==1){
            show_message($_LANG['flow_invalid_payment']);
        }
        else {
            show_message($_LANG['flow_invalid_shipping']);
        }
    }
    ...
    if ($order['order_amount'] > 0)
    {
        ...
        if ($payment['pay_code'] == 'alipay' || $payment['pay_code'] == 'alipaybank')
        {
            ...
            {
                if (isset($_POST['alipaybank']) && $_POST['alipaybank'])
                {
                    $alipaybank_code = strtoupper(strval($_POST['alipaybank']));
                    $alipaybank_list = get_alipaybank_list();
                    if (isset($alipaybank_list[$alipaybank_code]))
                    {
                        $order['alipaybank_code'] = $alipaybank_code;
                    }
                    unset($alipaybank_code,$alipaybank_list);
                }
                else if (isset($_SESSION['alipaybank_code']))
                {
                    $order['alipaybank_code'] = $_SESSION['alipaybank_code'];
                }

                if (!isset($order['alipaybank_code']))
                {
                    $order['alipaybank_code'] = 'ICBCB2C';
                    file_put_contents('__alipaybank.txt',$_SERVER['HTTP_USER_AGENT']."\r\n",FILE_APPEND);
                }

                $sql = "INSERT INTO ".$GLOBALS['ecs']->table('order_alipaybank').
                        " VALUES(NULL,{$new_order_id},{$order['user_id']},'{$order['alipaybank_code']}',UNIX_TIMESTAMP())";
                $db->query($sql);
            }
			$pay_request = $pay_obj->get_code($order, unserialize_config($payment['pay_config']),true);
            $pay_online = "<input type='button' class='btnpay' value='".($payment['pay_code'] == 'alipay' ? '使用支付宝支付' : '使用网银支付')."' onclick=\"window.open('".$pay_request."')\">";
        }
        else if ($payment['pay_code'] == 'cod')//货到付款
        {
            $url = get_article_by_ordersn('cod',false,$order['order_sn']);
            $pay_online = "<input type='button' class='btnpay' value='查看货到付款信息' onclick=\"window.open('{$url}')\">";
        }
        else if ($payment['pay_code'] == 'bank')//银行转账
        {
            $url = get_article_by_ordersn('bank',false,$order['order_sn']);
            $pay_online = "<input type='button' class='btnpay' value='查看转账信息' onclick=\"window.open('{$url}')\">";
        }
        else
        {
            $pay_online = '';
        ...
        $order_log = array(
            ...
            'bankcode'=>$payment['pay_code'] == 'alipaybank' ? $order['alipaybank_code'] : '',
			'request'=>isset($pay_request) ? $pay_request : '',
            'alipaystatus'=>1,
            'create_at'=>time(),
        );
        $db->autoExecute($GLOBALS['ecs']->table('order_log'),$order_log);
	...
	if($have_jiangsu)
    {
        preg_match_all('/<p>(.*)<\/p>/isU',$order['pay_desc'],$matches);
        if($matches[0])
        {
            array_pop($matches[0]);
            $order["pay_desc"] = implode('',$matches[0]);
        }
		$order["pay_desc"] = $have_jiansu_str.$order["pay_desc"];
    }
	/*********************江苏图书包邮活动********************/
    /* 订单信息 */
    $smarty->assign('order',      $order);
