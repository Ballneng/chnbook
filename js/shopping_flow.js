...
function selectShipping(obj)
{
  ...
  for (i = 0; i < theForm.elements.length; i ++ )
  {
    ...
    {
    	if(theForm.elements[i].attributes['isCod'].value == '1')
		{
		  if (supportCod == 0)
		  {
...
function checkOrderForm(frm)
{
  ...
  for (i = 0; i < frm.elements.length; i ++ )
  {
    ...
    if (frm.elements[i].name == 'payment' && frm.elements[i].checked)
    {
      ...
      bankcode = frm.elements[i].className;
    }
  }
  ...
  //当用户选择支付宝网银时,检查是否已经选择网银
  if (bankcode == 'alipaybank')
  {
      var f = false;
      for (i = 0; i < frm.elements.length; i ++ )
      {
          if (frm.elements[i].name == 'alipaybank' && frm.elements[i].checked)
          {
              f = true;
              break;
          }
...
function select_alipaybank(code)
{
    Ajax.call('flow.php?step=select_alipaybank', 'code=' +code, function(result){
        if (result == 'err'){
            alert('选择银行出错,请重新选择银行!!');
            return false;
        }
    }, 'POST');
}
