...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        //2012-06-30    qianlei     添加出版社和主编
        $gj = array(2,9,6,17,7,18,19,14,23,20,22,21,3,25,24,26,29,30,32,13);
        $goods['press'] = '河海大学出版社';
        if ($goods['cat_id'] == 1)
            $goods['chief_editor'] = '江苏公务员考试命题研究中心';
        else if (in_array($goods['cat_id'],$gj))
            $goods['chief_editor'] = '国家公务员考试网教材编写组';
        else
            $goods['chief_editor'] = '公务员考试网教材编写组';

		if ($goods['goods_id']==42){
			$goods['chief_editor'] = '江苏公务员考试网教材编写组';
		}
        ...
        //热销产品
        $smarty->assign('hot_goods',       array_slice(get_recommend_goods('hot'),0,4));
