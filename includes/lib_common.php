...
function get_index_msg_list($num, $start)
{
    /* 获取留言数据 */
    $msg = array();
    ...
    $sql .= "WHERE STATUS =1 AND comment_type =0) ORDER BY add_time DESC ";
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $msg[$rows['msg_time']]['user_name'] = htmlspecialchars($rows['user_name'] ? $rows['user_name'] : '匿名网友');
...
function get_order_msg_list($num=4, $start=0,$id=0)
{
    /* 获取留言数据 */
    $msg = array();
