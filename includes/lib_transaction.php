...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            if ($payment_info['pay_code'] == 'alipaybank')
            {
                //支付宝网银
                $sql = "SELECT `bankcode` FROM ".$GLOBALS['ecs']->table('order_alipaybank') .
                    "WHERE order_id=".$order['order_id'];
                if ($bankcode = $GLOBALS['db']->getOne($sql))
                {
                    $order['alipaybank_code'] = $bankcode;
