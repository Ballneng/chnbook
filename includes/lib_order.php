...
/**
 * 包邮条件:如果用户只选择购买了一本书,且这本书属于包邮商品中,则包邮
 * @param mixed $goods_info(既可以是单独ID,也可以是购物车)  前者表示直接传ID,后者表示传的是购物车信息
 */
function is_goods_baoyou($goods_info)
{
    if (is_array($goods_info) && count($goods_info)==1)
    {
        $goods = current($goods_info);
        $goods_id = isset($goods['goods_id']) ? $goods['goods_id'] : 0;
    }
    else if (is_int($goods_info))
        $goods_id = $goods_info;
    ...
    //包邮的产品
    $baoyou_goods_id = array(
        //45,//2012年事业单位考试复习教材
        //40,//2012年河南公务员考试复习教材
        //4,//2013年广东公务员考试提前复习教材
        //5,//2013年江西公务员考试提前复习教材
        //10,//2013年湖北公务员考试提前复习教材
        //6,//2012年福建公务员考试复习教材
        //9,//2013年浙江公务员考试提前复习教材
        //30,//2012年四川公务员考试复习教材
        //15,//2012年内蒙古公务员考试复习教材
        //16,//2012年重庆公务员考试复习教材
        //31,//2012年吉林公务员考试复习教材
        //35,//2012年西藏公务员考试复习教材
        //42,//2013年江苏公务员考试提前复习教材火热发售中
        /*2012-08-17 qianlei 增加ID=47 */
        //47,//2013年浙江公务员考试复习教材
    );

    return  in_array($goods_id,$baoyou_goods_id) ? true : false;
}

/**
 * 如果是包邮，则货运方式中去掉ems和宅急送，并将韵达排在第一.其余情况和以前一样
 * @param array $shipping_list 如果为空，则代表只是检查是否是包邮
 */
function choose_shipping_by_baoyou($shipping_list=array())
{
    foreach ($shipping_list as $key => $row)
    {
        //去掉ems和宅急送且将韵达放第一个
        if ($row['shipping_id']==1 || $row['shipping_id']==10 || $row['shipping_id']==7 || $row['shipping_id']==15)
        {
    ...
    if ($tmp)
        array_unshift($shipping_list, $tmp);

    return $shipping_list;
}

/**
 * 添加货运服务范围,这里将服务范围写死在程序里面,以后改进
 * @author qianlei 2012-08-19
 * @params array $shipping_list  货运列表
 * @todo 将这些配置写入数据库
 */
function shipping_add_service_range($shipping_list)
{
    $service_config = array(
        1=>'查询宅急送派送范围http://www.zjs.com.cn/',
        2=>'顺丰派送范围，<a href="http://www.sf-express.com/cn/sc" target="_blank" style="color:red;">点击查询</a>',
        3=>'查询韵达派送范围 <a href="http://www.yundaex.com/" target="_blank" style="color:red;">点击查询</a><br>
			<font color="red">注意：</font>贵州仁怀市、青岛崂山区自即日因故暂停十天（10月13日-10月23日）；浙江宁波市北仑区网点暂停发货，何时开通，等待通知。',
        4=>'不在其它快递服务内可选择EMS国内特快专递',
        5=>'宅急送派送范围，<a href="http://www.zjs.com.cn/" target="_blank" style="color:red;">点击查询</a>',
        6=>'查询韵达配送范围，<a href="http://www.yundaex.com/www/index.html" target="_blank" style="color:red;">点击查询</a><br>
            注意：<font color="red">包邮快递默认发韵达快递，仅限于选择非货到付款的方式购买包邮教材的考生，偏远地区只能通过EMS快递邮寄，包邮金额限制为20元内，超出部分需要您支付运费差额。</font>',
    );

    foreach ($shipping_list as $key =>$shipping)
    {
        if ($shipping['shipping_code']=='zto')
            $tmp_val = $service_config[1];
        else if ($shipping['shipping_code']=='sf_express')
            $tmp_val = $service_config[2];
        else if ($shipping['shipping_code']=='yto')
            $tmp_val = $service_config[3];
        else if ($shipping['shipping_code']=='ems')
            $tmp_val = $service_config[4];
        else if ($shipping['shipping_code']=='sto_express')
            $tmp_val = $service_config[5];
        else if ($shipping['shipping_code']=='post_express')
            $tmp_val = $service_config[6];
        else
            $tmp_val = '';

        $shipping_list[$key]['service_range'] = $tmp_val;
    }
...
/**
 * 添加支付特别提示,这里将特别提示写死在程序里面,以后改进
 * @author qianlei 2012-08-19
 * @params array $payment_list  货运列表
 * @todo 将这些配置写入数据库
 */
function payment_add_special_tips($payment_list)
{
    $tips_config = array(
        1=>'支付宝：支付宝公司签约单位，安全快捷',
        2=>'仅宅急送快递和顺丰速运支持货到付款',
        3=>'线下通过ATM机转账或柜面付款',
        4=>'网上银行：各大银行的网上银行付款，支持信用卡',
    );

    foreach ($payment_list as $key => $payment)
    {
        if ($payment['pay_code'] == 'alipay')
            $tmp_val = $tips_config[1];
        else if ($payment['pay_code'] == 'cod')
            $tmp_val = $tips_config[2];
        else if ($payment['pay_code'] == 'bank')
            $tmp_val = $tips_config[3];
        else if ($payment['pay_code'] == 'alipaybank')
            $tmp_val = $tips_config[4];
        else
            $tmp_val = '';

        $payment_list[$key]['pay_special_tips'] = $tmp_val;
    }
...
function get_article_by_ordersn($type,$id_only,$order_sn='')
{
    $article_id = 0;
    if ($type == 'cod') //货到付款
    {
    ...
    {
        return '/article.php?type='.$type.'&order_sn='.$order_sn;
    }
}
?>