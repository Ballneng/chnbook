...
if ($special || !$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    if ($special)
    {
        ...
        if (!$order = order_info(0,$_GET['order_sn']))
        {
            ecs_header("Location: ./\n");
            exit;
        }

        $article_id = get_article_by_ordersn($_GET['type'],true);
    }

    $article = get_article_info($article_id,$special);
    ...
    if ($special)
    {
        $article['content'] = str_replace(
            array('{order_sn}','{order_amount}'),
            array($order['order_sn'],$order['formated_order_amount']),$article['content']
        );
    }
...
if ($special)
{
    $smarty->display('article_order.dwt');
}
else if(isset($article) && $article['cat_id'] > 2)
{
 ...
 * @param bool|int $special 是否特殊文章
 * @return  array
 */
function get_article_info($article_id,$special=false)
{
    /* 获得文章的信息 */
    if ($special)
    {
